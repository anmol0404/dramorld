...
import { AIODocument } from "./interfaces/aIO";
import { InviteUserDocument } from "./interfaces/inviteUser";
import { OngoingDocument } from "./interfaces/ongoingDocument";
import { AIOSearchCriteria } from "./interfaces/searchCriteria";
import { SortDocument } from "./interfaces/sort";
import { UserDocument } from "./interfaces/user";
import { InviteService } from "./inviteService";
import { AIOModel } from "./models/aIOModel";
import { MessageDocument, MessageModel } from "./models/messageModel";
import { OngoingModel } from "./models/ongoingModel";
...
class MongoDB {
  ...
  async getAIOByShareId(shareId: number) {
    return await this.AIOModel.findOne({ shareId });
  }
  ...
  async searchAIO(
    ...
    if (!criteria.title || criteria.title.length < 2) {
      console.log("Please provide a valid search criteria.");
      return undefined;
    }
    const normalizedTitle = criteria.title;
  ...
  async getAllAIOPaginated(
    ...
    return await this.AIOModel.find(query).skip(skip).limit(pageSize).exec();
  }
  ...
  async getInviteUser(userId: string): Promise<InviteUserDocument | null> {
